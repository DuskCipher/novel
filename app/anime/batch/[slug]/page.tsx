...
export default function AnimeBatchPage() {
  ...
  useEffect(() => {
    ...
    const fetchBatch = async () => {
      try {
        const res = await getAnimeBatch(slug);
      ...
      <div className="w-full max-w-4xl mx-auto flex flex-col items-center pt-8 px-4 sm:px-6">
        ...
        <div className="flex flex-wrap justify-center gap-2 mb-10">
          {(detail.genreList || detail.genre_list || []).map((g: any, i: number) => (
            <Link key={i} href={`/anime/genre/${g.genreId || g.slug}`} className="bg-[#1C1D2A] border border-zinc-800 text-zinc-300 hover:text-white text-xs font-medium px-4 py-1.5 rounded-lg transition-colors">

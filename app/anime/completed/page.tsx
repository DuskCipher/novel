...
export default function AnimeCompletedPage() {
  ...
  const fetchCompleted = async (p: number) => {
    setLoading(true);
    try {
      const res = await getAnimeCompleted(p);
      const items = res?.data?.animeList || res?.animeList || res?.completed_anime || (Array.isArray(res?.data) ? res.data : []);
      const mapped = items.map((item: any) => ({
        title: item.title,
        poster: item.poster || item.thumb,
        href: `/anime/detail/${item.animeId || item.id}`,
        type: 'SERIES',
        status: 'TAMAT',
        year: '2025',
    ...
    <div className="flex-1 flex flex-col min-h-screen text-white pb-24 font-sans w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-4">
      ...
      <div className="flex items-center gap-3 mb-8">
        <div className="w-10 h-10 rounded-full bg-[#2A2B3D] flex items-center justify-center">
          <CheckSquare size={24} className="text-[#60a5fa]" />
        </div>
        ...
        {loading ? (
          ...
          <>
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4">
              {list.map((item, i) => (
                <AnimeCard3 key={i} item={item} href={item.href} type="movie" />
              ))}
            </div>

...
export default function AnimeGenreDetailPage() {
  ...
  const fetchByGenre = async (p: number) => {
    setLoading(true);
    try {
      const res = await getAnimeByGenre(slug, p);
      const items = res?.data?.animeList || res?.animeList || (Array.isArray(res?.data) ? res.data : []);
        ...
        <div className="flex overflow-x-auto custom-scrollbar gap-2 mb-8 pb-2">
          {formatTabs.map((tab, idx) => {
            ...
            return (
              <button
                key={idx}
                onClick={() => router.push(`/anime/genre/${tab.id}`)}
